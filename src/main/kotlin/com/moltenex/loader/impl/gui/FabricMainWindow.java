...
import com.moltenex.loader.impl.gui.MoltenexStatusTree.MoltenexStatusButton;
import com.moltenex.loader.impl.gui.MoltenexStatusTree.FabricStatusNode;
import com.moltenex.loader.impl.gui.MoltenexStatusTree.MoltenexStatusTab;
import com.moltenex.loader.impl.gui.MoltenexStatusTree.FabricTreeWarningLevel;

import net.fabricmc.loader.impl.util.StringUtil;

//TODO go create good code for moltenex window

class FabricMainWindow {
	static Icon missingIcon = null;

	static void open(MoltenexStatusTree tree, boolean shouldWait) throws Exception {
	...
	private static void open0(MoltenexStatusTree tree, boolean shouldWait) throws Exception {
	...
	private static void createUi(CountDownLatch onCloseLatch, MoltenexStatusTree tree) {
		...
		if (tree.tabs.isEmpty()) {
			MoltenexStatusTab tab = new MoltenexStatusTab("Opening Errors");
			...
			MoltenexStatusTab tab = tree.tabs.get(0);
			contentPane.add(createTreePanel(tab.node, tab.filterLevel, icons), BorderLayout.CENTER);
		} else {
			JTabbedPane tabs = new JTabbedPane();
			contentPane.add(tabs, BorderLayout.CENTER);

			for (MoltenexStatusTab tab : tree.tabs) {
		...
		if (!tree.buttons.isEmpty()) {
			...
			for (MoltenexStatusButton button : tree.buttons) {
